...
IndexedDB обновляется последовательными миграциями из
`src/api/indexedDb/migrations`. Новую миграцию нужно добавить в массив в
`src/api/indexedDb/migrator.ts`; номер последней миграции автоматически
становится `indexedDbDatabaseVersion`. Существующие упражнения и очередь не
удаляйте без отдельной миграции данных. Новые упражнения с backend
подмешиваются в локальный кэш при входе и при восстановлении соединения.
